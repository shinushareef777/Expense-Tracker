...
async function getExpenseList(pageNum) {
  try {
    let response = await axios.get(
      `http://127.0.0.1:4000/api/expenses?page=${pageNum}`
    );
...
async function getExpense(id) {
  try {
    let response = await axios.get(`http://127.0.0.1:4000/api/expenses/${id}`);
...
async function deleteExpense(id) {
  try {
    await axios.delete(`http://127.0.0.1:4000/api/expenses/${id}`);
